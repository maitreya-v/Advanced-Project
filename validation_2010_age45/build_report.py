"""Build the 2010 age-45 validation reports without executing the graph."""
...
GRAPH = BASE.parent / "2010_45 yo.py"
...
methodology = {
    "scope": "2010_45 yo.py, 61 edges, using only the 25 supplied PDF files in 2010 ADHD Papers (24 unique documents; one exact duplicate). Direction, sign, and written reasoning were reviewed; numeric strength was excluded.",
    "age_scope": "Adult evidence was required for adult claims. Young-adult and general-adult evidence was treated as partial when it did not report an age-45 or midlife estimate. Pediatric findings were not transferred as adult causal proof.",
...
readme = f"""# 2010 age-45 graph validation

Validation of **2010_45 yo.py**: **{len(edges)} edges**, using only the **25 supplied PDF files (24 unique documents)** in `2010 ADHD Papers`.
...
Run `python validation_2010_age45/build_report.py`. The script parses but never executes or modifies the graph. It verifies the source edge text, PDF hashes, citations, JSON output, and Excel structure.
"""
