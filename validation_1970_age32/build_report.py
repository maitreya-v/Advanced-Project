...
GRAPH = BASE.parent / '1970_32 yo.py'
...
manifest = {'graph_file':str(GRAPH),'graph_sha256':original_hash,'graph_year':1970,'age':32,'edge_count':45,'paper_count':4,'reference_pmids':sorted(allowed),'validation_updated_utc':now,'documents':REVIEW['documents']}
methods = [
 ('Scope','1970_32 yo.py, 45 edges, using only four supplied papers (PMIDs 5538099, 5523436, 5491539, 4950109). Review direction, sign and reasoning; strength excluded.'),
 ...
 ('Age scope','Schain ages 7-13; Obler approximately 7-12; Wikler initial eligibility 5-15. None studies adult patients aged 32. Family histories mentioning relatives are not adult outcome studies. Childhood age trends are not used to contradict adult severity or impairment claims.'),
 ...
 ('Reproduce','Run python validation_1970_age32/build_report.py. The reviewed judgments are stored in fulltext_review.py and checked against the actual 1970 source on every run.')
]
...
summary={'graph':'1970_32 yo.py','edge_count':45,'updated_utc':now,'status_counts':counts,'partial_support_edges':[],'contradicted_edges':[],'documents':REVIEW['documents'],'complete_articles':3,'incomplete_articles':1}
...
readme='# 1970 age-32 graph validation\n\nExisting reports replaced using the correct **1970_32 yo.py** graph: **45 edges**, **four supplied references only**.\n\n'
readme+='## Status counts\n\n'+''.join(f'- `{s}`: {n}\n' for s,n in counts.items())
readme+='\n## Method and limitations\n\n'+'\n\n'.join('**'+k+'**: '+v for k,v in methods)
readme+='\n\n## Fixed status definitions\n\n'+''.join(f'- `{k}`: {v}\n' for k,v in definitions.items())
(BASE/'README.md').write_text(readme,encoding='utf-8')
checks={'graph':'1970_32 yo.py','edges_reviewed':45,'references_reviewed':4,'status_counts':counts,'original_sha256':original_hash,'original_unchanged':True,'graph_1990_unchanged':True,'all_edge_reasons_and_signs_preserved':True,'xlsx_xml_valid':True,'xlsx_json_rows_match':True,'xlsx_data_rows':45,'all_citations_in_four_paper_corpus':True,'compact_edge_fields':list(rows[0])}
