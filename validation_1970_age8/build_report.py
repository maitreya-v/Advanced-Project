"""Rebuild only existing 1970 validation outputs; never execute or modify the graph."""
...
GRAPH = BASE.parent / '1970_8 yo.py'
...
manifest = {'graph_file':str(GRAPH),'graph_sha256':original_hash,'graph_year':1970,'age':8,'edge_count':47,'paper_count':4,'reference_pmids':sorted(allowed),'validation_updated_utc':now,'documents':REVIEW['documents']}
methods = [
...
assert all((BASE/f).exists() for f in outputs), 'Only existing outputs may be overwritten.'
...
summary={'graph':'1970_8 yo.py','edge_count':47,'updated_utc':now,'status_counts':counts,'partial_support_edges':[35,40,43],'contradicted_edges':[4],'documents':REVIEW['documents'],'complete_articles':3,'incomplete_articles':1}
...
readme='# 1970 age-eight graph validation\n\nExisting reports replaced using the correct **1970_8 yo.py** graph: **47 edges**, **four supplied references only**.\n\n'
readme+='## Status counts\n\n'+''.join(f'- `{s}`: {n}\n' for s,n in counts.items())
readme+='\n## Method and limitations\n\n'+'\n\n'.join('**'+k+'**: '+v for k,v in methods)
readme+='\n\n## Fixed status definitions\n\n'+''.join(f'- `{k}`: {v}\n' for k,v in definitions.items())
(BASE/'README.md').write_text(readme,encoding='utf-8')
checks={'graph':'1970_8 yo.py','edges_reviewed':47,'references_reviewed':4,'status_counts':counts,'original_sha256':original_hash,'original_unchanged':True,'graph_1990_unchanged':True,'all_edge_reasons_and_signs_preserved':True,'xlsx_xml_valid':True,'xlsx_json_rows_match':True,'xlsx_data_rows':47,'all_citations_in_four_paper_corpus':True,'compact_edge_fields':list(rows[0])}
